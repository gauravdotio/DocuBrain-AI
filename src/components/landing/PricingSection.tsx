...
export const PricingSection: React.FC<PricingSectionProps> = ({ onRequestDemo }) => {
  ...
  return (
    <section id="pricing" className="py-20 sm:py-24 bg-slate-50 border-b border-slate-200/80">
          ...
          {tiers.map((tier, idx) => (
            <div
              ...
              className={`rounded-3xl p-8 flex flex-col justify-between transition-all relative ${
                tier.featured
                  ? "bg-white border-2 border-brand-500 shadow-xl scale-[1.02]"
                  : "bg-white border border-slate-200 shadow-card"
              }`}
            >
              {tier.badge && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-brand-500 text-white tracking-wider shadow-sm">
                  {tier.badge}
                </span>
              )}
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  {tier.featured && <Zap className="w-4 h-4 text-brand-500" />}
                </div>

                <p className="text-xs text-slate-500 mt-2 min-h-[32px]">{tier.description}</p>
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 space-y-3">
                  <span className="text-[11px] font-mono text-slate-400 uppercase tracking-wider block font-semibold mb-2">
